...
Dim i As Long
Dim j As Long
Dim Counts(1 To MAXHISTLEN, 1 To HHMAXCHAN) As Long 'actually unsigned long
...
For i = 1 To InpChannels 'fetch all histograms
    Retcode = HH_GetHistogram(Dev(0), Counts(1, i), i - 1, 0) 'must pass the first array element
        If Retcode < 0 Then
            ConsolePrint "HH_GetHistogram error " & CStr(Retcode) & vbCrLf
            GoTo Ex
        End If
    Integralcount = 0
    For j = 1 To MAXHISTLEN
...
Open "HISTOMODE.OUT" For Output Shared As #1
Dim Tmpstr As String
For i = 1 To MAXHISTLEN
    Tmpstr = ""
    For j = 1 To InpChannels
        Tmpstr = Tmpstr & CStr(Counts(i, j)) & "  "
    Next j
    Print #1, Tmpstr
Next i
Close
