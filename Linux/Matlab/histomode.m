...
% Demo for access to HydraHarp 400 Hardware via HHLIB.DLL v 1.2.
...
%
% Michael Wahl, PicoQuant, August 2009
...
fprintf('\nHydraHarp 400 HHLib.DLL Demo Application             PicoQuant 2009\n');
...
if (~libisloaded('HHlib'))    
    ...
    loadlibrary('/usr/local/lib/hh400/hhlib.so', 'hhlib.h', 'alias', 'HHlib');
else
...
[ret, Model, Partno] = calllib('HHlib', 'HH_GetHardwareInfo', dev(1), ModelPtr, PartnoPtr);
if (ret<0)
    fprintf('\nHH_GetHardwareInfo error %1d. Aborted.\n',ret);
    closedev;
	return;
else
	fprintf('\nFound model %s part number: %s', Model, Partno);             
end;
...
[ret] = calllib('HHlib', 'HH_SetSyncCFDLevel', dev(1), SyncCFDLevel);
if (ret<0)
    fprintf('\nHH_SyncSetCFDLevel error %ld. Aborted.\n', ret);
    closedev;
    return;
end;

[ret] = calllib('HHlib', 'HH_SetSyncCFDZeroCross', dev(1), SyncCFDZeroX);
if (ret<0)
...
for i=0:NumInpChannels-1 % we use the same input settings for all channels
	...
	[ret] = calllib('HHlib', 'HH_SetInputCFDLevel', dev(1), i, InputCFDLevel);
	if (ret<0)
	...
	[ret] = calllib('HHlib', 'HH_SetInputCFDZeroCross', dev(1), i, InputCFDZeroX);
	if (ret<0)
   	fprintf('\nHH_SetInptCFDZeroCross error %ld. Aborted.\n', ret);
   	closedev;
   	return;
   end;
